...
def message_to_be_processed(ctx):
  ...
  """
  assert not (ctx.author == bot.user or "out_channel" not in db.keys() or bot.get_channel(db["out_channel"]) != ctx.channel), "Command attempt in the wrong channel"
  return True
...
async def donate_cmd(ctx, user_receiver, amount):
  ...
  piflouz_handlers.update_piflouz(bot.user, qty=qty_tax, check_cooldown=False)
  piflouz_handlers.update_piflouz(user_receiver, qty=qty_receiver, check_cooldown=False)

  await ctx.send(f"{user_sender.mention} sent {amount} {Constants.PIFLOUZ_EMOJI} to {user_receiver.mention}")
...
@slash.slash(name="isLive", description="check if a certain streamer is live", guild_ids=Constants.GUILD_IDS, options=[
...
async def is_live_cmd(ctx, streamer_name):
  ...
  if r["data"] != []:
    ...
    await ctx.send(f"{streamer_name} is currently live on \"{title}\", go check out on http://www.twitch.tv/{streamer_name} ! {Constants.FUEGO_EMOJI}")
  else:
...
@slash.slash(name="balance", description=f"check how many piflouz you have. Kind of a low-cost Piflex", guild_ids=Constants.GUILD_IDS, options=[])
...
@slash.slash(name="get", description="for the lazy one", guild_ids=Constants.GUILD_IDS, options=[])
...
@slash.slash(name="cooldown", description="when your addiction is stronger than your sense of time", guild_ids=Constants.GUILD_IDS, options=[])
...
@slash.slash(name="piflex", description=f"when you have too many piflouz /!\ Costs {Constants.PIFLEX_COST} piflouz", guild_ids=Constants.GUILD_IDS, options=[])
...
@slash.slash(name="buyRankPiflex", description=f"flex with a custom rank /!\ Costs {Constants.PIFLEXER_COST} piflouz, lasts for {Constants.PIFLEXROLE_DURATION} seconds", guild_ids=Constants.GUILD_IDS, options=[])
...
@slash.slash(name="pilord", description="see how much you need to farm to flex with your rank", guild_ids=Constants.GUILD_IDS, options=[])
...
async def raffle_cmd(ctx, nb_tickets):
  ...
  # user doesn't have enough money
  assert piflouz_handlers.update_piflouz(ctx.author, qty=-price, check_cooldown=False), f"User {ctx.author} doesn't have enough money to buy {nb_tickets} tickets"
...
@slash.slash(name="store", description="buy fun upgrades", guild_ids=Constants.GUILD_IDS, options=[])
...
@slash.slash(name="powerups", description="see how powerful you are", guild_ids=Constants.GUILD_IDS, options=[])
...
@slash.subcommand(base="duel", name="challenge", description="Duel someone to earn piflouz!", guild_ids=Constants.GUILD_IDS, options=[
...
async def duel_challenge_cmd(ctx, user, amount, duel_type):
  ...
  assert piflouz_handlers.update_piflouz(ctx.author, qty=-amount, check_cooldown=False), "You don't have enough piflouz to do that"

  new_duel = utils.create_duel(ctx, user, amount, duel_type)
  db["duels"].append(new_duel)

  await ctx.send(f"{ctx.author.mention} challenged {user.mention} at {duel_type} betting {amount} {Constants.PIFLOUZ_EMOJI}! Use `/duel accept @{ctx.author.name}` to accept or `/duel deny @{ctx.author.name}` to deny.")
  await utils.update_piflouz_message(bot)


@slash.subcommand(base="duel", name="accept", description="Accept someone's challenge", guild_ids=Constants.GUILD_IDS, options=[
...
async def duel_accept_cmd(ctx, user):
  ...
  assert piflouz_handlers.update_piflouz(ctx.author, qty=-db["duels"][duel_index]["amount"], check_cooldown=False), "You don't have enough piflouz to do that"
  ...
  await ctx.send(f"{ctx.author.mention} accepted {user.mention}'s challenge! Use `/duel play {duel_type} @[opponent] [your move]`")
  await utils.update_piflouz_message(bot)


@slash.subcommand(base="duel", name="deny", description="Deny someone's challenge", guild_ids=Constants.GUILD_IDS, options=[
  create_option(name="user", description="The person who challenged you", required=True, option_type=option_type.USER)
])
async def duel_deny_cmd(ctx, user):
  ...
  piflouz_handlers.update_piflouz(user, qty=db["duels"][duel_index]["amount"], check_cooldown=False)

  del db["duels"][duel_index]
  await ctx.send(f"{ctx.author.mention} denied {user.mention}'s challenge!")
  await utils.update_piflouz_message(bot)


@slash.subcommand(base="duel", name="cancel", description="Cancel your challenge to someone", guild_ids=Constants.GUILD_IDS, options=[
...
async def duel_cancel_cmd(ctx, user):
  ...
  # Give back the money to the challenger
  piflouz_handlers.update_piflouz(ctx.author, qty=db["duels"][duel_index]["amount"], check_cooldown=False)

  del db["duels"][duel_index]

  await ctx.send(f"{ctx.author.mention} cancelled his/her challenge to {user.mention}, what a loser")
  await utils.update_piflouz_message(bot)
...
async def duel_play_shifumi_cmd(ctx, user, value):
  ...
  if move1 is not None and move2 is not None:
    ...
    if move1 == move2:
      ...
      piflouz_handlers.update_piflouz(player2, qty=db["duels"][duel_index]["amount"], check_cooldown=False)

      await ctx.send(f"{player1.mention} and {player2.mention} tied at {db['duels'][duel_index]['duel_type']}! They both played {move1}! They both got their money back")

    # Player 1 won
    elif win_shifumi[move1] == move2:
      piflouz_handlers.update_piflouz(player1, qty=money_if_win, check_cooldown=False)

      await ctx.send(f"{player1.mention} won at {db['duels'][duel_index]['duel_type']} against {player2.mention}, earning {money_if_win} {Constants.PIFLOUZ_EMOJI}! They played {move1} vs {move2}")
    
    # Player 2 won
    else:
      piflouz_handlers.update_piflouz(player2, qty=money_if_win, check_cooldown=False)

      await ctx.send(f"{player2.mention} won at {db['duels'][duel_index]['duel_type']} against {player1.mention}, earning {money_if_win} {Constants.PIFLOUZ_EMOJI}! They played {move2} vs {move1}")
...
@slash.slash(name="ranking", description="See how worthy you are", guild_ids=Constants.GUILD_IDS, options=[])

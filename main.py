...
async def on_ready():
  ...
  duel_status_cmd.add_check(message_to_be_processed)
...
async def duel_play_shifumi_cmd(ctx, user, value):
  ...
  if move1 is not None and move2 is not None:
    ...
    # Player 1 won
      ...
      piflouz_handlers.update_piflouz(bot.user.id, qty=money_tax, check_cooldown=False)
...
@slash.subcommand(base="duel", name="status", description="Check your ongoing duels", guild_ids=Constants.GUILD_IDS, options=[])
async def duel_status_cmd(ctx):
  """
...
async def on_raw_reaction_add(playload):
  ...
  if not (("twitch_notif_message_id" in db.keys() and message_id == db["twitch_notif_message_id"])
  or ("store_message_id" in db.keys() and message_id == db["store_message_id"])
  or ("piflouz_message_id" in db.keys() and message_id == db["piflouz_message_id"])
  or str(message_id) in db["random_gifts"]):
    return

  channel = bot.get_channel(playload.channel_id)
  message = await channel.fetch_message(playload.message_id)
  guild = bot.get_guild(playload.guild_id)

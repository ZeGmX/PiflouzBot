...
async def update_rank_pilord(bot):
    ...
    L = sorted(list(user_profile.get_inverted("piflouz_balance").items()), key=lambda key_val: -int(key_val[1]))
    # in case of ties
...
async def update_rank_piflex_master(bot):
    ...
    L = sorted(list(user_profile.get_inverted("discovered_piflex").items()), key=lambda key_val: -len(key_val[1]))

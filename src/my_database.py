...
class Element:
    ...
    def convert_to_element(item, parent_dict, parent_key, stop_parent_propagation=False):
        ...
        if type(item) is list:
            return ElementList(parent_dict, parent_key, item)
        elif type(item) is dict:
            return ElementDict(parent_dict, parent_key, item, stop_parent_propagation)
        else:
            return item
    ...
    def get_folder_path(self, root_folder):
        ...
        ----------
        root_folder (str):
            the folder where the whole database is stored
...
class ElementList(Element):
    ...
    def __add__(self, other):
        ...
        res (Element_list)
        """
        return ElementList(self.parent_dict, self.parent_key, self.element + other)
...
class ElementDict(Element):
    ...
    def keys(self):
        ...
        res (dict_keys)
        """
        return ElementDictKeys(self.element.keys(), self.parent_dict, self.parent_key, self.stop_parent_propagation)
    ...
    def values(self):
        ...
        if self.stop_parent_propagation:
            return ElementDictValues(self.element.values(), self.parent_dict, self.parent_key, self.stop_parent_propagation)
        return ElementDictValues(self.element.values(), self, self.parent_key, self.stop_parent_propagation)  # parent_key will be set for each item
    ...
    def items(self):
        ...
        return ElementDictItems(self.element.items(), self, self.parent_key, self.stop_parent_propagation)
    ...
    def save(self, root_folder):
        ...
        ----------
        root_folder (str):
            the folder where the dictionary will be saved
        """
    ...
    def save_key(self, key, root_folder):
        ...
        if isinstance(value, ElementDict):
    ...
    def load(self, folder):
        ...
        ----------
        folder (str):
            the folder where the dictionary will be loaded from
        """
...
class ElementDictKeys(ElementDictIterator):
    pass


class ElementDictValues(ElementDictIterator):
    def __iter__(self):
        self.iterator = iter(self.collection)
...
class ElementDictItems(ElementDictIterator):
    def __next__(self):
...
class MyDatabase(dict):
    ...
    def make_backup(self, parent_folder: str = "backups", folder: str = "database") -> None:
        ...
        ----------
        parent_folder (str):
            the parent folder where the backup will be stored
        folder (str):
            the name of the folder where the backup will be stored
        """
    ...
    def _load(self, folder: str = None) -> None:
        ...
        ----------
        folder (str):
            the folder where the database will be loaded from
        """
        if folder is None:
            folder = self.folder

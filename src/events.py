  ...
  """
    ...
    async def on_end(self, bot):
        ...
        if len(participation) > 0:
            ...
            piflouz_handlers.update_piflouz(bot.me.id,
                                            qty=tax_value,
    ...
    """
        ...
        channel = await bot.get_channel(db["out_channel"])
        embed = await self.get_embed_raffle(bot)
        raffle_message = await channel.get_message(
            db["current_event_message_id"])
    ...
    """
        ...
        prize = floor(nb_tickets * self.ticket_price * (100 - self.tax_ratio) /
                      100)
        return prize
...
class Birthday_event(Event):
    ...
    """
        ...
        embed = Embed(
            ...
            thumbnail=EmbedImageStruct(url=Constants.PIBOU4BIRTHDAY_URL),
            description=
            f"Congratulations, you managed to bake {nb_cakes} cakes. Pibot will invest the earned {nb_cakes * self.reward_per_cake} {Constants.PIFLOUZ_EMOJI} in the next event! 👀",
            color=Color.from_rgb(255, 255, 255).value  # white
        )
        return embed
    ...
    output:
      interactions.Button
    """
        return Button(style=ButtonStyle.SECONDARY,
                      label=str(nb),
    ...
    async def actions_every_5min(self, bot):
        ...
        components = [
            self.get_component(emoji, nb)
            for emoji, nb in zip(self.ingredients, qty)
        ]
        role_notif = Role(id=Constants.BIRTHDAY_NOTIF_ROLE_ID)
        msg = await out_channel.send(
            f"{role_notif.mention} A new cake ingredient delivery has appeared! But you can only take one type so chose carefully",
            components=components)
        db["last_birthday_delivery"] = {
            "id": int(msg.id),
            "qty": {e: nb
                    for e, nb in zip(self.ingredients, qty)}
        }
    ...
    """
        ...
        nb_cakes = min(inv[e] // self.ingredients_per_cake[e]
...
class Birthday_raffle_event(Event):
  ...
  """
    ...
    """
        ...
        raffle_message = await channel.get_message(
            db["current_event_message_id"])
        await raffle_message.edit(embeds=embed, components=button)
    ...
    """
        ...
        if "birthday_raffle_participation" in db.keys() and len(
            ...
            embed.add_field(name="Current participants",
                            value=val,
                            inline=False)
            embed.add_field(
                name="Total prize",
    ...
    """
        res = Button(style=ButtonStyle.SECONDARY,

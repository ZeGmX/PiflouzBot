...
def load_chess_database(db_path="chess_database/"):
    ...
    db_path (str, optional):
        The path to the folder containing the database. Defaults to "chess_database/".

    Returns
    -------
        res (dict):
        A mapping of ratings to a tuple of (shardsize, shardfilename)
    """
    map_path = None
...
def lookup_problem(mapping, rating=1500):
    ...
    mapping (dict):
        A mapping of ratings to a tuple of (shardsize, shardfilename)
    rating (int, optional):
        The target problem rating. Defaults to 1500

    Returns
    -------
    res (dict):
        The selected problem, with all the attributes as in the base database
    """
    rating = rating // 100 * 100  # rating r contains problems from r to r + 99

...
class SubseqChallenge:
    ...
    def check_default(self, answer, all_clean_words=None, check_if_real_word=True):
        ...
        ----------
        answer (str)
        all_clean_words (List[str]):
            list of all accepted (clean) words
        check_if_real_word (bool):
            whether to check if the answer is in the list of all accepted words
        ...
        if all_clean_words is None:
            all_clean_words = SubseqChallenge.get_word_list("src/events/assets/all_french_words_clean.txt")
    ...
    def new(length):
        ...
        Subseq_challenge
        List[int]:
            how many solutions there are for each level
        str:
            example solution that solves all levels
        """
        easy_words = SubseqChallenge.get_word_list("src/events/assets/easy_french_words.txt")
        all_clean_words = SubseqChallenge.get_word_list("src/events/assets/all_french_words_clean.txt")
        all_words = SubseqChallenge.get_word_list("src/events/assets/all_french_words.txt")

        i = 1
        res = SubseqChallenge.attempt_find_new(easy_words, all_words, all_clean_words, length)
        while res is None:
            i += 1
            res = SubseqChallenge.attempt_find_new(easy_words, all_words, all_clean_words, length)
    ...
    def attempt_find_new(easy_words, all_words, all_clean_words, length):
        ...
        ----------
        easy_words (List[str]):
            list of the top 2000 most common words
        all_words (List[str]):
            list of all words  = all accepted answers
        all_clean_words (List[str]):
            list of all (cleaned) words
        length (int):
            length of the subsequence

        Returns
        -------
        subseq (Subseq_challenge):
            the subsequence object
        all_sols (List[List[str]]):
            list of solutions for each level
        """
        # Condition 1: there exists a solution in the top 2000 most common words
        solution_lvl1 = choice(easy_words)
        solution_lvl1_clean = SubseqChallenge._clean_word(solution_lvl1)
    ...
    def get_word_list(path):
        ...
        ----------
        path (str):
            path to the file containing the word list. Each word should be on a separate line, with no header
